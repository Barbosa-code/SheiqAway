...
function getLoggedUser() {
  try { return JSON.parse(localStorage.getItem("loggedUser")) || null; } catch { return null; }
}
function getUserKey(u) { return u?.email || u?.username; }
...
function getUserPoints(id) {
  if (typeof window.getUserPoints === "function") return window.getUserPoints(id);
...
function updateUserPoints(id, delta) {
  if (typeof window.updateUserPoints === "function") return window.updateUserPoints(id, delta);
  const users = JSON.parse(localStorage.getItem("users")) || [];
...
function redeemPoints(id, pointsToUse, rate = { pts: 100, eur: 5 }) {
  if (typeof window.redeemPoints === "function") return window.redeemPoints(id, pointsToUse, rate);
...
function ensureIdsAndMigrateUsernames() {
  ...
  for (const t of all) {
    // dar ID se faltar
    if (t.id == null) { t.id = Date.now() + Math.random(); changed = true; }

    // se não houver username no item e existe utilizador logado, associar ao atual (compat)
    if ((t.username == null || t.username === "") && key) { t.username = key; changed = true; }
  }
...
function setUserCartTickets(items) {
  ...
  const others = all.filter(t => !(t.username == null || t.username === key || t.username === user.username));
  setAllCartTickets([...others, ...items.map(it => ({ ...it, username: (it.username ?? key) }))]);
}
...
function renderCart() {
  ...
  if (cartSummary) {
    ...
    items.forEach(t => {
      ...
      li.innerHTML = `<span>${t.from} → ${t.to}</span><span>€${getPriceNum(t).toFixed(2)}</span>`;
  ...
  if (totalToPayEl)    totalToPayEl.textContent = Math.max(0, subtotal - discount).toFixed(2);
...
function wireLoyaltyAndCheckout() {
  ...
  checkoutBtn?.addEventListener("click", () => {
    ...
    // gasta pontos agora
    const { used, discount } = redeemPoints(key, appliedPoints, { pts: 100, eur: 5 });
...
function enforceBundlePricingForUser() {
  ...
  for (const item of mine) {
    if (!item.packageId || !Array.isArray(item.packageTripIds) || !item.tripId) continue;
    if (!groups.has(item.packageId)) {
      groups.set(item.packageId, { items: [], expected: new Set(item.packageTripIds) });
    }
  ...
  for (const [pkgId, group] of groups.entries()) {
    ...
    for (const item of group.items) {
      ...
      if (!valid) {
        ...
        if (typeof item.originalPrice === "number" && item.price !== item.originalPrice) {
...
window.addEventListener("storage", (e) => {
  if (["cartTickets", "loggedUser", "users"].includes(e.key)) enforceBundlePricingForUser(); renderCart();
});
